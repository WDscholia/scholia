...
def redirect_q(q):
    ...
    return redirect( url_for(method, q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'), code=302))
...
def show_arxiv(arxiv):
    ...
    if len(qs) > 0:
        q = qs[0]
        return redirect( url_for('app.show_work', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed')), code=302)
...
def show_arxiv_to_quickstatements():
    ...
    if len(qs) > 0:
        ...
        q = qs[0]
        return render_template('arxiv_to_quickstatements.html',
                               arxiv=arxiv, q=q,  SPARQLEndPointEmbed = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_author(q):
    ...
    return render_template('author.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'),  first_initial=first_initial,
                           last_name=last_name)
...
def show_author_missing(q):
    ...
    """
    return render_template('author_missing.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_author_random():
    ...
    """
    q = random_author()
    return redirect(url_for('app.show_author', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed')), code=302)
...
def show_authors(qs):
    ...
    return render_template('authors.html', qs=qs, webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_award(q):
    ...
    """
    return render_template('award.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_award_missing(q):
    ...
    """
    return render_template('award_missing.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def redirect_cas(cas):
    ...
    if len(qs) > 0:
        q = qs[0]
        return redirect(url_for('app.show_chemical', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed')), code=302)
    return render_template('404.html')
...
def redirect_lipidmaps(lmid):
    ...
    if len(qs) > 0:
        q = qs[0]
        if (len(lmid) < 12):
            return redirect(url_for('app.show_chemical_class', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed')), code=302)
        else:
            return redirect(url_for('app.show_chemical', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed')), code=302)
    return render_template('404.html')
...
def redirect_atomic_symbol(symbol):
    ...
    if len(qs) > 0:
        q = qs[0]
        return redirect(url_for('app.show_chemical_element', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed')), code=302)
    return render_template('404.html')
...
def redirect_cordis(cordis):
    ...
    if len(qs) > 0:
        q = qs[0]
        return redirect(url_for('app.show_project', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed')), code=302)
    return render_template('404.html')
...
def show_catalogue(q):
    ...
    """
    return render_template('catalogue.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_clinical_trial_empty():
    ...
    """
    return render_template('clinical_trial_empty.html', webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_clinical_trial(q):
    ...
    """
    return render_template('clinical_trial.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_country(q):
    ...
    """
    return render_template('country.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_country_topic(q1, q2):
    ...
    """
    return render_template('country_topic.html', q1=q1, q2=q2, q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_disease(q):
    ...
    """
    return render_template('disease.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_event(q):
    ...
    """
    return render_template('event.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_event_series(q):
    ...
    """
    return render_template('event_series.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def redirect_issn(issn):
    ...
    if len(qs) > 0:
        q = qs[0]
        return redirect(url_for('app.show_venue', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed')), code=302)
    return render_template('404.html')
...
def show_location(q):
    ...
    """
    return render_template('location.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_location_topic(q1, q2):
    ...
    """
    return render_template('location_topic.html', q1=q1, q2=q2, q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def redirect_wikipathways(wpid):
    ...
    if len(qs) > 0:
        q = qs[0]
        return redirect(url_for('app.show_pathway', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed')), code=302)
    return render_template('404.html')
...
def redirect_ror(rorid):
    ...
    if len(qs) > 0:
        q = qs[0]
        return redirect(url_for('app.show_organization', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed')), code=302)
    return render_template('404.html')
...
def show_organization(q):
    ...
    """
    return render_template('organization.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_organization_topic(q1, q2):
    ...
    """
    return render_template('organization_topic.html', q1=q1, q2=q2, q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_organization_missing(q):
    ...
    """
    return render_template('organization_missing.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_printer(q):
    ...
    """
    return render_template('printer.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_printer_empty():
    ...
    """
    return render_template('printer_empty.html', webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_protein(q):
    ...
    """
    return render_template('protein.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_project(q):
    ...
    """
    return render_template('project.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_gene(q):
    ...
    """
    return render_template('gene.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_taxon(q):
    ...
    """
    return render_template('taxon.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_q_to_bibliography_templates():
    ...
    return render_template('q_to_bibliography_templates.html',
                           q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'), 
                           wikitext=wikitext)
...
def show_software(q):
    ...
    """
    return render_template('software.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_topic(q):
    ...
    """
    return render_template('topic.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_topic_missing(q):
    ...
    """
    return render_template('topic_missing.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_chemical_element(q):
    ...
    """
    return render_template('chemical_element.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_chemical_class(q):
    ...
    """
    return render_template('chemical_class.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def redirect_twitter(twitter):
    ...
    if len(qs) > 0:
        q = qs[0]
        return redirect(url_for('app.redirect_q', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed')),code=302)
    return render_template('404.html')
...
def show_venue(q):
    ...
    """
    return render_template('venue.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed')) 
...
def show_venue_missing(q):
    ...
    """
    return render_template('venue_missing.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_venues(qs):
    ...
    """
    qs = Q_PATTERN.findall(qs)
    return render_template('venues.html', qs=qs, webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed') )
...
def show_series(q):
    ...
    """
    return render_template('series.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed') )
...
def show_pathway(q):
    ...
    """
    return render_template('pathway.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed') )
...
def show_publisher(q):
    ...
    """
    return render_template('publisher.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed') )
...
def show_sponsor(q):
    ...
    """
    return render_template('sponsor.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_use(q):
    ...
    """
    return render_template('use.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
...
def show_work(q):
    ...
    """
    return render_template('work.html', q=q,  webserviceURL = app.config['config'].get('servers', 'SPARQLEndPointEmbed'))
